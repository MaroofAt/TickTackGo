...
class PointsStatistics extends StatefulWidget {
  ...
  const PointsStatistics(
      {required this.workspaceId, required this.workspaceName, super.key});
...
class _PointsStatisticsState extends State<PointsStatistics> {
  ...
  void initState() {
    super.initState();
    BlocProvider.of<PointsCubit>(context).getPointsStatistics(widget.workspaceId);
  }
      ...
      body: BlocBuilder<PointsCubit, PointsState>(
        builder: (context, state) {
          if (state is GettingPointsStatisticsSucceededState) {
            return WorkspaceGetPointsStatisticsModelDetailScreen(
              state.getPointsStatisticsModel,
            );
          }
          return Center(
